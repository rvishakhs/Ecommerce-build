...
function Productcard({grid, data}) {
    ...
    const addwishlist = (id) => { 
        ...
        {data && data?.map((item, index) => (
            <div key={index} className={`${location.pathname === "/store" ? `col-span-${grid}  ${grid === 2 ? "flex flex-col" : "flex flex-row"} relative  group bg-white rounded-xl my-2 ` 
            ...
            <div className={`${location.pathname === "/store" ? `max-w-[70%] pt-4 ${grid === 2 ? "px-2 py-2" : "pr-8  py-2"}` : "space-y-1 px-3"} `}>
                <p className='font-medium text-red-800 text-sm '>{item.brand}</p>
                <Link to="/product/:id">
                    <p className='font-bold text-black text-sm'>{item.tittle}</p>            
                </Link>
                <StarRatings
                    ...
                    starSpacing="2px"
                    />     
                <p className={`font-normal text-sm text-gray-500 text-justify py-2 ${grid === 10 ? "block" : "hidden"}`} dangerouslySetInnerHTML={{ __html : item?.description}}></p>       
